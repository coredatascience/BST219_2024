...
```{r}
co2_tidy <- co2_tidy %>% mutate(month = as.numeric(month))
...
```{r}
yearly_avg <- co2_tidy %>% group_by(year) %>%
...
```{r}
combined_df <- co2_tidy %>% left_join(yearly_avg, by = "year") %>%
...
```{r}
combined_df %>% ggplot(aes(month, residual, color = year)) + 
	geom_line()
```
---
...
```{r}
top <- Salaries %>% filter(yearID == 2016) %>%
	select(playerID, salary) %>%
	right_join(top, by = "playerID")
```
